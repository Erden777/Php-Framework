...
<div class="container well span-6 offset-2 mt-4">
	...
	<div class="row">
        <div class="col-md-2" >
		    ...
		    <img width="160px" height="160px" src="/storage/picture/{{session()->get('company')->pictureURL}}" class="img-circle">
            @endif
                ...
                @foreach($vacancies as $vacancy)
                    @if($vacancy->company_id==session()->get('company')->id)
                        <div class="jumbotron mt-1">
                            ...
                            <a class="btn btn-lg btn-danger" href="{{route('Delete-Vacancy' , $vacancy->id)}}" role="button">Delete</a>
                        </div> 
                    @endif
                 @endforeach

           <a href="{{route('user_details' , session()->get('company')->id )}}" class="btn btn-primary btn-sm" >Update</a>
        </div>
	</div>
</div>
